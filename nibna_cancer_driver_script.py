...
from nibna.passenger_genes_filtering import passenger_genes_filtering, sample_filter, CCDS_data_processing, TCGA_data_processing, critical_nodes_filter
...
if __name__ =="__main__":
    ...
    logger.info("Reading CCDS data")
    CCDS_data = CCDS_data_processing("CCDS.current_p38.txt", "CCDS_protein.current_p38.faa")

    logger.info("Reading TCGA mutation status")
    TCGA = TCGA_data_processing("data_mutations_extended_TCGA.txt", CCDS_data)
    ...
    refilter = input("Do you want to use custom dataset for passenger genes filtering (True/False): ")
    logger.info("Identifying passenger genes")
    passenger_genes = passenger_genes_filtering(TCGA, CCDS_data, gold_standard_cgc, refilter = False)
    ...
    critical_nodes = critical_nodes_filter(critical_nodes, passenger_genes)
